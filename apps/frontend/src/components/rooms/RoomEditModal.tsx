...
export default function RoomEditModal({ room, isOpen, onClose, onRoomUpdated }: RoomEditModalProps) {
  const [formData, setFormData] = useState<UpdateRoomDto>({
    name: room.name,
    area: room.area || 0,
    wastePercent: room.wastePercent || 0,
  ...
  const loadRoomProducts = async () => {
    try {
      const response = await roomProductsApi.getRoomProducts(room.id);
      const roomProducts = response.data || [];
      
      const formattedProducts: Product[] = roomProducts.map((rp: RoomProduct) => ({
        id: rp.id,
        productId: rp.productId,
        productName: rp.product?.nameBg || rp.product?.nameEn || 'Unknown Product',
        quantity: rp.quantity,
        unitPrice: rp.unitPrice,
        discount: rp.discount || 0,
        unit: rp.product?.unit || 'м²',
        wastage: rp.wastePercent || 0
      }));
      
      setProducts(formattedProducts);
    } catch (err) {
      console.error('Failed to load room products:', err);
    }
  };
  ...
  const removeProduct = async (index: number) => {
    ...
    if (product.id) {
      // Ако продуктът вече е запазен в базата, го изтриваме
      try {
        await roomProductsApi.deleteRoomProduct(product.id);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Обновяваме или добавяме продуктите
      for (const product of products) {
        if (product.id) {
          // Обновяваме съществуващ продукт
          await roomProductsApi.updateRoomProduct(product.id, {
          ...
          <div className="border-t pt-6">
            ...
            {products.length > 0 && (
              <div className="space-y-4">
                <h4 className="font-medium text-gray-900">Продукти в стаята:</h4>
